ALTER TABLE public.students
  ...
  ADD COLUMN IF NOT EXISTS aadhaar_edited_fields text[] NOT NULL DEFAULT '{}';

CREATE UNIQUE INDEX IF NOT EXISTS students_institute_aadhaar_hash_key
  ON public.students (institute_id, aadhaar_hash)
...
CREATE OR REPLACE FUNCTION public.submit_admission_application(
  ...
  _address text, _program text, _stream text, _photo_path text, _preferred_contact text,
  _intent text, _token_amount numeric, _institute_slug text DEFAULT NULL::text,
  _aadhaar_last4 text DEFAULT NULL, _aadhaar_hash text DEFAULT NULL,
  _aadhaar_verified boolean DEFAULT false, _aadhaar_edited_fields text[] DEFAULT '{}')
RETURNS uuid
LANGUAGE plpgsql
