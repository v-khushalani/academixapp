...
function findConflicts(candidate: { day_of_week: number; start_time: string; end_time: string; room?: string | null; faculty_id?: string | null; batch_id?: string | null; id?: string }, all: SlotRow[]) {
  return all.filter((s) => {
    ...
    const sameFaculty = candidate.faculty_id && s.faculty_id && s.faculty_id === candidate.faculty_id;
    const sameBatch = candidate.batch_id && s.batch_id && s.batch_id === candidate.batch_id;
    return Boolean(sameRoom || sameFaculty || sameBatch);
  });
}

function conflictReason(a: { room?: string | null; faculty_id?: string | null; batch_id?: string | null }, b: SlotRow): string {
  const reasons: string[] = [];
  if (a.room && b.room && a.room.trim().toLowerCase() === b.room.trim().toLowerCase()) reasons.push(`room ${b.room}`);
  if (a.faculty_id && b.faculty_id === a.faculty_id) reasons.push(`teacher ${b.faculty?.full_name ?? ""}`.trim());
...
function TimetablePage() {
  ...
  const conflictIds = useMemo(() => {
    ...
    for (let i = 0; i < slots.length; i++) {
      for (let j = i + 1; j < slots.length; j++) {
        ...
        if (sameRoom || sameFaculty || sameBatch) { bad.add(a.id); bad.add(b.id); }
      }
    }
    return bad;
  }, [slots]);
  ...
  function onDropCell(dayIdx: number, band: { start: string; end: string }, ev: DragEvent) {
    ...
    if (conflicts.length) {
      const reasons = conflicts.map((c) => conflictReason(candidate, c)).filter(Boolean).join("; ");
  ...
  function shareToWhatsApp() {
    ...
    slots.forEach((s) => { const arr = byDay.get(s.day_of_week) ?? []; arr.push(s); byDay.set(s.day_of_week, arr); });
    ...
    DAY_INDEX.forEach((dow) => {
      ...
      rows.forEach((r) => {
        ...
        const parts = [time, r.batch?.name, r.subject, r.faculty?.full_name && `👨‍🏫 ${r.faculty.full_name}`, r.room && `🚪 ${r.room}`].filter(Boolean);
        lines.push(`• ${parts.join(" · ")}`);
      });
    });
    if (lines.length === 1) { toast.info("No slots to share yet."); return; }
      ...
      <PageHeader
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            <Button size="sm" variant="outline" className="gap-1.5" onClick={shareToWhatsApp}><Share2 className="h-4 w-4" />Share to WhatsApp</Button>
            {canWrite && <Button size="sm" className="gap-1.5" onClick={() => { setEditing(null); setDefaultDay(1); setDialogOpen(true); }}><Plus className="h-4 w-4" />New slot</Button>}
          </div>
        }
      />
      <PageBody>
        {conflictIds.size > 0 && (
          <div className="mb-3 flex items-start gap-2 rounded-lg border border-destructive/40 bg-destructive/5 p-3 text-sm text-destructive">
            ...
            <p>{conflictIds.size} slot(s) have conflicts (same room, teacher, or batch overlapping). They're highlighted in red — edit or remove them.</p>
          </div>
        )}
                        ...
                        return (
                          <td key={dow}
                            ...
                            <div className="flex min-h-[56px] flex-col gap-1">
                              {cells.map((s) => (
                                <div key={s.id} className={`group rounded-md border p-1.5 ${conflictIds.has(s.id) ? "border-destructive/60 bg-destructive/10" : "border-primary/20 bg-primary/5"}`}>
...
function ClassBuilder({ batches, faculty }: { batches: Batch[]; faculty: Faculty[] }) {
  ...
  function reset() { setBatchId(""); setFacultyId(""); setSubject(""); setRoom(""); }

  return (
    <aside className="space-y-3 rounded-lg border border-border bg-card p-3">
      <div>
        <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Class builder</p>
        <p className="mt-0.5 text-[11px] text-muted-foreground">Fill all four, drag the card onto any cell.</p>
      </div>
      ...
      <div className="space-y-1.5">
        ...
        <Select value={facultyId} onValueChange={(v) => { setFacultyId(v); const f = faculty.find((x) => x.id === v); if (f?.subject && !subject) setSubject(f.subject); }}>
          <SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Select teacher" /></SelectTrigger>
          <SelectContent>{faculty.map((f) => <SelectItem key={f.id} value={f.id}>{f.full_name}{f.subject ? ` · ${f.subject}` : ""}</SelectItem>)}</SelectContent>
        </Select>
      </div>
      <div className="space-y-1.5">
        <Label className="text-xs">Subject</Label>
        <Input value={subject} onChange={(e) => setSubject(e.target.value)} className="h-8 text-xs" placeholder="Physics" />
      </div>
      <div className="space-y-1.5">
      ...
      <div className="flex gap-2">
        <Button type="button" variant="ghost" size="sm" className="h-7 w-full text-xs" onClick={reset}>Reset</Button>
      </div>
    </aside>
  );
}
