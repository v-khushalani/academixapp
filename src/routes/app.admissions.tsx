...
function AdmissionsPage() {
  ...
  return (
    <>
      <PageHeader title="Admissions" description="Leads pipeline, pending applications, and the public QR." />
      <PageBody>
        <Tabs value={tab} onValueChange={setTab} className="w-full">
          ...
          <TabsContent value="leads" className="mt-4"><LeadsBoard canWrite={canWrite} /></TabsContent>
          <TabsContent value="applications" className="mt-4"><ApplicationsList canWrite={canWrite} /></TabsContent>
          <TabsContent value="qr" className="mt-4"><QrPanel /></TabsContent>
          <TabsContent value="how" className="mt-4"><HowItWorks /></TabsContent>
        </Tabs>
      </PageBody>
    </>
  );
  ...
  return (
    <div>
      <div className="mb-3 flex items-center justify-between">
        ...
        {canWrite && <Button size="sm" className="gap-1.5" onClick={() => setOpen(true)}><Plus className="h-4 w-4" />Add lead</Button>}
      </div>
...
function ApplicationsList({ canWrite }: { canWrite: boolean }) {
  ...
  if (data.length === 0) return (
    <div className="rounded-lg border border-dashed border-border bg-card p-8 text-center">
      ...
      <p className="mt-1 text-xs text-muted-foreground">Share the QR from the "Public QR" tab to start collecting applications.</p>
    </div>
  );
            ...
            <tr key={s.id}>
              <td className="px-4 py-3">
                <button className="text-left" onClick={() => setPreview(s)}>
                  ...
                  <p className="text-xs text-muted-foreground">{s.phone ?? "—"} · {s.admission_no}</p>
                </button>
              </td>
              <td className="px-4 py-3">{s.class ?? "—"}</td>
              <td className="px-4 py-3 capitalize">
                {s.program ?? "—"}{s.stream ? ` · ${s.stream.toUpperCase()}` : ""}
              </td>
              <td className="px-4 py-3">
                <p className="text-sm">{s.father_name || s.mother_name || "—"}</p>
                <p className="text-xs text-muted-foreground">{s.father_phone || s.mother_phone || "—"}</p>
              </td>
              <td className="px-4 py-3 text-xs text-muted-foreground">
                {s.onboarding_completed_at ? new Date(s.onboarding_completed_at).toLocaleDateString() : "—"}
              </td>
              <td className="px-4 py-3">
                {canWrite && (
                  <div className="flex justify-end gap-1.5">
                    <Button size="sm" variant="outline" onClick={() => setPreview(s)}>View</Button>
                    <Button size="sm" variant="outline" className="gap-1 text-destructive" onClick={() => approveMut.mutate({ id: s.id, decision: "rejected" })}>
                      <X className="h-3.5 w-3.5" />Reject
                    </Button>
                    <Button size="sm" className="gap-1 bg-success text-success-foreground hover:bg-success/90" onClick={() => approveMut.mutate({ id: s.id, decision: "approved" })}>
...
function ApplicantPreview({ student, onClose }: { student: Student | null; onClose: () => void }) {
  ...
  useMemo(() => { setPhotoUrl(null); if (student?.photo_path) studentsApi.signedPhotoUrl(student.photo_path).then(setPhotoUrl); }, [student]);
  if (!student) return null;
  return (
    <Dialog open onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader><DialogTitle>Application · {student.full_name}</DialogTitle></DialogHeader>
        <div className="grid gap-4 sm:grid-cols-[140px_1fr]">
          <div>
            {photoUrl ? (
              <img src={photoUrl} alt={student.full_name} className="h-36 w-full rounded-md border border-border object-cover" />
            ) : (
...
function QrPanel() {
  ...
  function copy() { navigator.clipboard.writeText(url).then(() => toast.success("Link copied")); }
  return (
    <div className="grid gap-6 md:grid-cols-[auto_1fr]">
      <div className="rounded-lg border border-border bg-card p-6 text-center">
        ...
        <div className="mt-4 flex justify-center gap-2">
          <Button size="sm" variant="outline" className="gap-1.5" onClick={copy}><Copy className="h-4 w-4" />Copy link</Button>
          <Button size="sm" variant="outline" className="gap-1.5" asChild><a href={url} target="_blank" rel="noreferrer"><ExternalLink className="h-4 w-4" />Open</a></Button>
          <Button size="sm" variant="outline" className="gap-1.5" onClick={() => window.print()}><Printer className="h-4 w-4" />Print</Button>
        </div>
      </div>
      <div className="space-y-3 text-sm">
        <h3 className="text-base font-semibold">How to use this QR</h3>
        <ol className="list-decimal space-y-1.5 pl-5 text-muted-foreground">
          <li>Print this QR and stick it at the reception or hand it to parents at a school visit.</li>
          ...
          <li>The submission shows up under the <b>Applications</b> tab as "Pending".</li>
          <li>Your reception / admin reviews and approves or rejects. Approved applicants become active students.</li>
        </ol>
      </div>
    </div>
  );
  ...
  return (
    <div className="prose prose-sm max-w-none text-sm">
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <p className="mt-2 text-muted-foreground">Think of it as three lanes that feed each other:</p>

        <div className="mt-4 grid gap-4 md:grid-cols-3">
          <Lane n="1" title="Leads pipeline (Kanban)">
            Enquiries from parents you meet or receive calls from. Track them from <i>New</i> → <i>Contacted</i> → <i>Visit</i> → <i>Demo</i> → <i>Enrolled</i>. Update the stage in one click. This is your day-to-day sales board.
          </Lane>
          <Lane n="2" title="Applications (self-serve)">
            The parent fills the full form themselves — either via the Quick Admit link you send, or by scanning the public QR at your reception. Applications land here as <b>Pending</b>. Your admin approves or rejects. Approved applicants become students automatically.
          </Lane>
          <Lane n="3" title="Public QR">
            One QR code printed at reception. Anyone scans → fills form → application lands in the queue. Zero data entry for staff, zero cost.
          </Lane>
        </div>

        <h4 className="mt-6 font-semibold">Recommended workflow</h4>
        <ul className="mt-2 list-disc space-y-1 pl-5 text-muted-foreground">
          <li><b>Parent visits or calls:</b> receptionist adds a <b>Lead</b> and shares the QR / Quick Admit link.</li>
          <li><b>Parent fills the form on their phone.</b> Application enters the <b>Applications</b> tab.</li>
          <li><b>Admin reviews the photo + details</b> → clicks Approve. Student becomes active and can be assigned a batch on the Students page.</li>
          <li><b>Move the lead</b> to "Enrolled" and you're done.</li>
        </ul>
  ...
  return (
    <div className="rounded-md border border-border bg-background p-4">
      <div className="mb-1.5 flex items-center gap-2">
        <Badge variant="secondary" className="bg-primary/10 text-primary">{n}</Badge>
