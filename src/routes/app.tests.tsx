...
function TestsPage() {
  ...
  const columns: DTColumn<Row>[] = [
    ...
    },
    {
      ...
      header: "Type",
      value: (r) => r.type,
      cell: (r) => <span className="uppercase text-muted-foreground">{r.type}</span>,
    },
    {
      key: "subject",
      header: "Subject",
    ...
    { key: "max_marks", header: "Max", sortable: true, value: (r) => r.max_marks },
    {
      ...
      cell: (r) => (
        <Badge
          variant="secondary"
          className={
            r.status === "completed" ? "bg-success/10 text-success" : "bg-accent text-primary"
          }
        >
          {r.status}
        </Badge>
...
function TestDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  const qc = useQueryClient();
  const [f, setF] = useState<TestInsert>({
    title: "",
    type: "quiz",
    max_marks: 100,
      ...
      <DialogContent className="max-w-lg">
        ...
        >
          ...
          <div className="space-y-1.5">
            ...
            <Select
              value={f.type ?? "quiz"}
              onValueChange={(v) => setF({ ...f, type: v as TestInsert["type"] })}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {["quiz", "unit", "midterm", "final", "mock", "practice"].map((t) => (
          ...
          <div className="space-y-1.5 sm:col-span-2">
            <Label>Status</Label>
            <Select
              value={f.status ?? "scheduled"}
              onValueChange={(v) => setF({ ...f, status: v as TestInsert["status"] })}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {["scheduled", "ongoing", "completed", "cancelled"].map((t) => (
                  <SelectItem key={t} value={t}>
                    {t}
                  </SelectItem>
                ))}
