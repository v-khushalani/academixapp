...
export function BatchFormDialog({ open, onOpenChange, batch }: Props) {
  ...
  useEffect(() => {
    if (batch) setF({
      name: batch.name, capacity: batch.capacity, status: batch.status,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader><DialogTitle>{isEdit ? "Edit batch" : "New batch"}</DialogTitle></DialogHeader>
        <form onSubmit={(e: FormEvent) => { e.preventDefault(); mutation.mutate(f); }} className="grid gap-3 sm:grid-cols-2">
          <F label="Name" cls="sm:col-span-2"><Input value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })} required /></F>
          ...
          <F label="Start date"><Input type="date" value={f.start_date ?? ""} onChange={(e) => setF({ ...f, start_date: e.target.value || undefined })} /></F>
          <F label="End date"><Input type="date" value={f.end_date ?? ""} onChange={(e) => setF({ ...f, end_date: e.target.value || undefined })} /></F>
