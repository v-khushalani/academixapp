...
export function FacultyFormDialog({ open, onOpenChange, faculty }: Props) {
  ...
  useEffect(() => {
    ...
    else if (open) setF({ full_name: "", email: "", phone: "", qualification: "", subject: "", status: "active" });
  }, [faculty, open]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader><DialogTitle>{isEdit ? "Edit faculty" : "Add faculty"}</DialogTitle></DialogHeader>
        <form onSubmit={(e: FormEvent) => { e.preventDefault(); mutation.mutate(f); }} className="grid gap-3 sm:grid-cols-2">
          <F label="Full name" cls="sm:col-span-2"><Input value={f.full_name} onChange={(e) => setF({ ...f, full_name: e.target.value })} required /></F>
          <F label="Subject"><Input value={f.subject ?? ""} onChange={(e) => setF({ ...f, subject: e.target.value })} placeholder="Physics, Maths…" /></F>
          <F label="Qualification"><Input value={f.qualification ?? ""} onChange={(e) => setF({ ...f, qualification: e.target.value })} placeholder="M.Sc, B.Ed" /></F>
          ...
          <F label="Joining date"><Input type="date" value={f.joining_date ?? ""} onChange={(e) => setF({ ...f, joining_date: e.target.value || undefined })} /></F>
          <F label="Status">
          ...
          <F label="Notes" cls="sm:col-span-2"><Input value={f.notes ?? ""} onChange={(e) => setF({ ...f, notes: e.target.value })} /></F>
